...
prepare_directories() {
  ...
  printf '%s\n' "${group_chmods[@]+"${group_chmods[@]}"}" > /tmp/odyssey_group_chmods.txt
  printf '%s\n' "${user_chmods[@]+"${user_chmods[@]}"}" > /tmp/odyssey_user_chmods.txt
  printf '%s\n' "${chmods[@]+"${chmods[@]}"}" > /tmp/odyssey_chmods.txt
  printf '%s\n' "${chowns[@]+"${chowns[@]}"}" > /tmp/odyssey_chowns.txt
  printf '%s\n' "${chgrps[@]+"${chgrps[@]}"}" > /tmp/odyssey_chgrps.txt
  printf '%s\n' "${mkdirs[@]+"${mkdirs[@]}"}" > /tmp/odyssey_mkdirs.txt
  printf '%s\n' "${mkdirs_user_only[@]+"${mkdirs_user_only[@]}"}" > /tmp/odyssey_mkdirs_user_only.txt
}
